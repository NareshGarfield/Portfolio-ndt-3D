...
const NavButton = ({x, y, label, link, icon, newTab, labelDirection="right"}) => {
  ...
  return (
    ...
    >
        ...
        <span className='absolute hidden peer-hover:block px-2 py-1 left-full
        mx-2 top-1/2 -translate-y-1/2 bg-background text-foreground text-sm rounded-md shadow-lg whitespace-nowarp'>
            {label}
        </span>
        </span>
        ...
        <span className='relative w-14 h-14 p-4 hover:text-accent'>
            {getIcon(icon)}
            <span className='peer bg-transparent absolute top-0 left-0 w-full h-full' />
        <span className={clsx('absolute hidden peer-hover:block px-2 py-1 left-full mx-2 top-1/2 -translate-y-1/2 bg-background text-foreground text-sm rounded-md shadow-lg whitespace-nowarp', labelDirection === 'left' ?
